...
def create_fill_QC_df(directory, output_dir):
    # Initialize QC_df
    column_names = ["First_Last", "Screen_ID", 
                "MGK_R2_neg|lfc", 
                 ...
                 "p_file_numobs",
                 "g_file_numobs"]
    QC_df = pd.DataFrame(columns=column_names)
    ...
    for subdir, dirs, files in os.walk(directory):
        ...
        for file in files:
            ...
            file_path = Path(subdir + os.sep + file)
            ...
            if file_path.exists() and file_path.is_file() and file_path.suffix == '.csv':
                ...
                # Read the file into a DataFrame based on its extension and change column names to lowercase
                if f'_pDZ' in file_path.name:
                    pDZ_df = pd.read_csv(file_path)
                    print(f"Reading {file} into dataframe.")
                    for col in pDZ_df.columns:
                        pDZ_df.rename(columns={col: col.lower()}, inplace=True)
                if f'_gDZ' in file_path.name:
                    gDZ_df = pd.read_csv(file_path)
                    print(f"Reading {file} into dataframe.")
                    for col in gDZ_df.columns:
                        gDZ_df.rename(columns={col: col.lower()}, inplace=True)
                if f'_pMGK' in file_path.name:
                    pMGK_df = pd.read_csv(file_path)
                    print(f"Reading {file} into dataframe.")
                    for col in pMGK_df.columns:
                        pMGK_df.rename(columns={col: col.lower()}, inplace=True)
                if f'_gMGK' in file_path.name:
                    gMGK_df = pd.read_csv(file_path)
                    print(f"Reading {file} into dataframe.")
                    for col in gMGK_df.columns:
                        gMGK_df.rename(columns={col: col.lower()}, inplace=True)
                '''        
                elif file_path.suffix == '.xlsx':
                    if f'_pDZ' in file_path.name:
                        pDZ_df = pd.read_excel(file_path)
                        print(f"Reading {file} into dataframe.")
                    if f'_gDZ' in file_path.name:
                        gDZ_df = pd.read_excel(file_path)
                        print(f"Reading {file} into dataframe.")
                    if f'_pMGK' in file_path.name:
                        pMGK_df = pd.read_excel(file_path)
                        print(f"Reading {file} into dataframe.")
                    if f'_gMGK' in file_path.name:
                        gMGK_df = pd.read_excel(file_path)
                        print(f"Reading {file} into dataframe.")
                '''
            if pDZ_df is not None and gDZ_df is not None:
                #Use calc_stats() with pDZ_df and gDZ_df with DZ_R2 and DZ_slope lists.
                calc_stats(pDZ_df, gDZ_df, DZ_R2, DZ_slope, match_column = 'DZ', graph = True, file_name = file)
                print(f"Calculated stats for pDZ and gDZ tables.")
            if pMGK_df is not None and gMGK_df is not None:
                calc_stats(pMGK_df, gMGK_df, MGK_R2, MGK_slope, match_column = 'MGK', graph = True, file_name = file)
                print(f"Calculated stats for pMGK and gMGK tables.")
            
            
            # Get the parent directory and sub-directory names
        ...
        #print(f"Filling calculated stats from {file_path}")
        if pDZ_df is not None and gDZ_df is not None:
            for key, value in DZ_R2.items():
                QC_df.at[(parent_dir, sub_dir), f"DZ_R2_{key}"] = value
            for key, value in DZ_slope.items():
                QC_df.at[(parent_dir, sub_dir), f"DZ_slope_{key}"] = value
            if "numobs" in pDZ_df.columns and "numobs" in gDZ_df.columns:
                QC_df.at[(parent_dir, sub_dir), f"p_file_numobs"] = pDZ_df[f"numobs"].median()
                QC_df.at[(parent_dir, sub_dir), f"g_file_numobs"] = gDZ_df[f"numobs"].median()
        if pMGK_df is not None and gMGK_df is not None:
            for key, value in MGK_R2.items():
                QC_df.at[(parent_dir, sub_dir), f"MGK_R2_{key}"] = value
            for key, value in MGK_slope.items():
                QC_df.at[(parent_dir, sub_dir), f"MGK_slope_{key}"] = value
            if "num" in pMGK_df.columns and "num" in gMGK_df.columns:
                QC_df.at[(parent_dir, sub_dir), f"p_file_numobs"] = pMGK_df[f"num"].median()
                QC_df.at[(parent_dir, sub_dir), f"g_file_numobs"] = gMGK_df[f"num"].median()
        else:
            print(f"No values were calculated from dataframe pairs.")
            
    output_path = Path(output_dir) / "5-23-24_screen_QA_analysis.csv"
    QC_df.to_csv(output_path)
    return QC_df
                                                                                                                                                                                                                                                         
def calc_stats(df1: pd.DataFrame, df2: pd.DataFrame, R_list: list, slope_list: list, match_column: str, file_name: str, graph: bool = False):
    if match_column == 'DZ':
        join_column = 'gene'
    elif match_column == 'MGK':
        join_column = 'id'
    else:
    ...
    if match_column is not None:
        # Match rows by the determined column
        df1 = df1.set_index(join_column)
        df2 = df2.set_index(join_column)
        df1, df2 = df1.align(df2, join='inner')
        df1 = df1.reset_index()
        df2 = df2.reset_index()
    ...
    if graph:
        ...
        for i, column in enumerate(data_columns):
            if column in df1.columns and column in df2.columns:
                ...
                ax.plot(X, reg.predict(X), color='red', label=f"y = {slope}x + {reg.intercept_}")
                ax.set_xlabel(df1)
                ax.set_ylabel(df2)
                ax.set_title(f"Linear Regression of ({column})")
                ax.legend()
                ax.grid(True)
            else:
                print(f"Column '{column}' does not exist in one or both dataframes.")

        # Save the entire figure as an HTML file
        output_dir = "output"
        os.makedirs(output_dir, exist_ok=True)
        output_file = os.path.join(output_dir, file_name)
        plt.savefig(output_file, format="pdf")

        print(f"Linear regression plots saved as {output_file}")


data_path = "C:\\Users\\spejo\\Documents\\BI_Pipeline\\4_For_upload"
output_folder = "C:\\Users\\spejo\\Box\\DDR_Brow\\Datasets\\DDR\\5_QA_QC"
test_folder = "C:\\Users\\spejo\\Documents\\BI_Pipeline"
create_fill_QC_df(data_path, output_folder)df = df[~df['total_authors'].str.endswith("_")]
